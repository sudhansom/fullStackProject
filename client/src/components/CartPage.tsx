...
function CartPage() {
  ...
  const cart: ProductDocument[] = JSON.parse(
    ...
    for (let i = 1; i < products.length; i++) {
      ...
      while (a) {
        if (allKeys.includes(products[i]._id)) {
          if (
            myOrder.orderItem[products[i]._id].variant[0]['brand'] ===
              products[i].variant[0]['brand'] &&
            myOrder.orderItem[products[i]._id].variant[0]['size'] ===
              products[i].variant[0]['size'] &&
            myOrder.orderItem[products[i]._id].variant[0]['color'] ===
              products[i].variant[0]['color']
          ) {
  ...
  console.log(
    'images',
    myOrder.orderItem[allProducts[0]].variant[0],
    myOrder.totalPrice
  )
  const goBack = () => {
    //eslint-disable-next-line
  ...
  return (
    <div className="homePage">
      ...
      <Card className="cartContainer">
        <Row>
          <Col>
            <Button variant="info" onClick={goBack} style={{ width: '100%' }}>
              &larr; Back
            </Button>
          </Col>
        </Row>
          ...
          allProducts.map((elem, index) => {
            return (
              <Row key={index}>
                ...
                <Col>
                  <img
                    src={myOrder.orderItem[elem].images[0]}
                    height="60px"
                    width="100px"
                  ></img>
                </Col>
                ...
                <Col>
                  {myOrder.orderItem[elem].price *
                    myOrder.orderItem[elem].quantity}
                </Col>
                <Col>
                  {myOrder.orderItem[elem].variant[0]['brand']}/
                  {myOrder.orderItem[elem].variant[0]['size']}/
                  {myOrder.orderItem[elem].variant[0]['color']}
                </Col>
        ...
        <hr />
        <Row className="heading">
          <Col>Total quantity: {myOrder.totalQuantity}</Col>{' '}
          <Col>Total price: {myOrder.totalPrice}</Col>
        </Row>
